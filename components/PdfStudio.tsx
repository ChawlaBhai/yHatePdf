...
import { ArrowDown, ArrowLeft, ArrowUp, Check, Copy, Crop, Download, FilePlus2, GripVertical, Highlighter, LoaderCircle, Plus, RotateCcw, RotateCw, ShieldCheck, Stamp, Trash2, Type, Upload, X } from "lucide-react";
...
export default function PdfStudio(){
  ...
  function addAction(){
    try{
      ...
      else if(tool==="signature"){if(!text.trim()&&!signatureImage)throw new Error("Type or draw a signature.");action={id,type:tool,targetIds:targets,text:text.trim(),image:signatureImage||undefined,position:signaturePosition};}
  ...
  return <main className="pdf-studio page-container">
    ...
    <section className="studio-shell">
      ...
      <aside className="studio-inspector"><div className="studio-pane-title"><div><strong>ACTIONS</strong><span>STACK EDITS</span></div></div><div className="studio-tool-grid">{([["watermark",Stamp],["page-numbers",Type],["header-footer",FilePlus2],["signature",Type],["annotation",Highlighter],["crop",Crop],["remove-metadata",ShieldCheck]] as const).map(([type,Icon])=><button key={type} className={tool===type?"active":""} onClick={()=>setTool(type)}><Icon/>{actionName[type]}</button>)}</div><div className="inspector-fields"><label>ACTION SCOPE<select value={scope} onChange={event=>setScope(event.target.value as typeof scope)}><option value="selected">Selected pages ({selected.size})</option><option value="all">All pages ({plan.length})</option></select></label>
        ...
        {tool==="signature"&&<><label>TYPED SIGNATURE<input value={text} onChange={event=>setText(event.target.value)} placeholder="Your name"/></label><canvas ref={canvas} width={480} height={150} onPointerDown={beginDraw} onPointerMove={draw} onPointerUp={endDraw} onPointerCancel={endDraw} aria-label="Draw a signature"/><button className="clear-drawing" onClick={()=>{canvas.current?.getContext("2d")?.clearRect(0,0,480,150);setSignatureImage("")}}>CLEAR DRAWING</button><label>PLACEMENT<select value={signaturePosition} onChange={event=>setSignaturePosition(event.target.value as typeof signaturePosition)}><option value="bottom-right">Bottom right</option><option value="bottom-left">Bottom left</option><option value="top-right">Top right</option><option value="top-left">Top left</option></select></label><small>Visible signature only; not a certificate-based digital signature.</small></>}
